...
echo ""
echo -e "${BLUE}🗂️  整理 Shell 腳本${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

# 創建目錄
echo -e "${BLUE}📁 創建目標目錄...${NC}"
...
# 移動已完成的維護腳本
echo -e "${BLUE}📦 移動維護腳本...${NC}"
moved_count=0

if [ -f "cleanup_root_scripts.sh" ]; then
    mv cleanup_root_scripts.sh scripts/maintenance/
    echo -e "${GREEN}✅ cleanup_root_scripts.sh${NC} → scripts/maintenance/"
    ((moved_count++))
fi

if [ -f "cleanup_test_scripts.sh" ]; then
    mv cleanup_test_scripts.sh scripts/maintenance/
    echo -e "${GREEN}✅ cleanup_test_scripts.sh${NC} → scripts/maintenance/"
    ((moved_count++))
fi

if [ -f "fix_dhcp_timezone.sh" ]; then
    mv fix_dhcp_timezone.sh scripts/maintenance/
    echo -e "${GREEN}✅ fix_dhcp_timezone.sh${NC} → scripts/maintenance/"
    ((moved_count++))
fi

if [ -f "organize_root_docs.sh" ]; then
    mv organize_root_docs.sh scripts/maintenance/
    echo -e "${GREEN}✅ organize_root_docs.sh${NC} → scripts/maintenance/"
    ((moved_count++))
fi

if [ -f "reorganize_docs_phase2.sh" ]; then
    mv reorganize_docs_phase2.sh scripts/maintenance/
    echo -e "${GREEN}✅ reorganize_docs_phase2.sh${NC} → scripts/maintenance/"
    ((moved_count++))
fi

echo ""

# 檢查 deploy_ansible_inventory.sh
if [ -f "deploy_ansible_inventory.sh" ]; then
    echo -e "${YELLOW}⚠️  deploy_ansible_inventory.sh 保留在根目錄${NC}"
    echo "   └─ 建議確認功能狀態後再決定是否移動"
    echo ""
fi

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e "${GREEN}✨ 整理完成！共移動 ${moved_count} 個腳本${NC}"
echo ""

# 顯示根目錄保留的腳本
echo -e "${BLUE}📋 根目錄保留的腳本：${NC}"
echo ""
...
echo ""
echo -e "${BLUE}📂 scripts/ 目錄結構：${NC}"
echo ""
tree scripts/ -L 2 2>/dev/null || find scripts/ -type f -name "*.sh" | sort

echo ""
echo -e "${BLUE}📝 說明：${NC}"
echo ""
echo "  • 已移動的腳本：已完成的維護和整理腳本"
echo "  • 保留的腳本：系統啟動、功能性和仍在使用的腳本"
echo ""
echo "  如需執行維護腳本："
echo "  ./scripts/maintenance/cleanup_root_scripts.sh"
echo ""

# 生成報告
cat > docs/development/SHELL_SCRIPTS_CLEANUP_REPORT.md << 'EOF'
# Shell 腳本整理報告
...
echo -e "${GREEN}✅ 已生成報告: docs/development/SHELL_SCRIPTS_CLEANUP_REPORT.md${NC}"
echo ""
